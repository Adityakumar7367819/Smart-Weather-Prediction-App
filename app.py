...
# --- Load compressed model and features ---
...
model_path_zst = os.path.join(current_dir, "weather_model.zst")
model_path_pkl = os.path.join(current_dir, "weather_model.pkl")
features_path = os.path.join(current_dir, "weather_features.pkl")

try:
    if os.path.exists(model_path_zst):
        with open(model_path_zst, "rb") as f:
            dctx = zstd.ZstdDecompressor()
            model = pickle.loads(dctx.decompress(f.read()))
        st.success("✅ Loaded model from weather_model.zst")
    elif os.path.exists(model_path_pkl):
        model = joblib.load(model_path_pkl)
        st.success("✅ Loaded model from weather_model.pkl")
    else:
        st.warning("⚠️ No model file found. Using heuristic fallback.")
...
# --- Safe model loading (suppresses yellow warnings) ---

try:
    current_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(current_dir, "weather_model.zst")
    features_path = os.path.join(current_dir, "weather_features.pkl")
    ...
    if os.path.exists(model_path):
        try:
            ...
            model = None
            st.info("ℹ️ Model file found but could not be read. Using fallback heuristic.")
    else:
        model = None
        st.info("ℹ️ Model file not found. Using fallback heuristic.")
    ...
    model = None
    feature_names = None
    st.info("ℹ️ Model loading skipped due to unexpected error.")
